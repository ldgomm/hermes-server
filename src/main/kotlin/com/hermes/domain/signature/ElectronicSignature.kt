...
data class ElectronicSignature private constructor(
    ...
    init {
        if (id.isBlank()) throw DomainRuleViolation("Electronic signature id cannot be blank.")
        if (organizationId.isBlank()) throw DomainRuleViolation("Electronic signature organization id cannot be blank.")
        if (storageKey.isBlank()) throw DomainRuleViolation("Electronic signature storage key cannot be blank.")
        if (passwordSecretRef.isBlank()) throw DomainRuleViolation("Electronic signature password secret reference cannot be blank.")
        if (subject.isBlank()) throw DomainRuleViolation("Electronic signature subject cannot be blank.")
        if (issuer.isBlank()) throw DomainRuleViolation("Electronic signature issuer cannot be blank.")
        if (uploadedBy.isBlank()) throw DomainRuleViolation("Electronic signature uploadedBy cannot be blank.")
        if (!validFrom.isBefore(validTo)) throw DomainRuleViolation("Electronic signature validFrom must be before validTo.")
    }
