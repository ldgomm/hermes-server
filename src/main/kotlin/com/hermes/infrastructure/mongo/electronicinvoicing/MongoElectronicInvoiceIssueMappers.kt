...
object MongoElectronicInvoiceIssueMappers {
    ...
    fun toDocument(record: ElectronicInvoiceIssueRecord): Document =
        Document(MongoDocumentFields.ID, record.id)
            .append(MongoDocumentFields.ORGANIZATION_ID, record.organizationId)
            .append("branchId", record.branchId)
            .append("emissionPointId", record.emissionPointId)
            .append("saleId", record.saleId)
            .append("environment", record.environment.storageValue)
            .append("documentType", record.documentType.storageValue)
            .append("establishmentCode", record.series.establishmentCode)
            .append("emissionPointCode", record.series.emissionPointCode)
            .append("series", record.series.value)
            .append("documentNumber", record.documentNumber)
            .append("accessKey", record.accessKey.value)
            .append("authorizationNumber", record.authorizationNumber)
            ...
            .append("signedXmlObjectKey", record.signedXmlObjectKey)
    ...
    fun fromDocument(document: Document): ElectronicInvoiceIssueRecord =
    ...
    private fun messageToDocument(message: SriMessage): Document =
        Document("identifier", message.identifier)
            .append("message", message.message)
            .append("additionalInfo", message.additionalInfo)
            .append("type", message.type.name)

    private fun Document.messages(): List<SriMessage> =
        (this["sriMessages"] as? List<*>)
            .orEmpty()
    ...
    private fun classificationToDocument(classification: SriErrorClassification): Document =
        Document("category", classification.category.name)
            .append("recoverability", classification.recoverability.name)
    ...
    private fun classificationFromDocument(document: Document): SriErrorClassification =
    ...
    private fun Document.optionalDocument(fieldName: String): Document? =
        this[fieldName] as? Document
