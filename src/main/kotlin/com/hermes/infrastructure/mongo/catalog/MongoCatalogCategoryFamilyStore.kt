...
private class MongoCatalogCategoryRepository(database: MongoDatabase) : CatalogCategoryRepository {
    ...
    override fun search(query: CatalogCategorySearchQuery): List<CatalogCategory> {
        ...
        query.query?.trim()?.takeIf { it.isNotBlank() }?.let { text ->
            filters += or(regex("name", text, "i"), regex("normalizedName", text.lowercase(), "i"), regex("code", text.lowercase(), "i"))
        }
        val finalFilter = if (filters.isEmpty()) Document() else and(filters)
        return collection.find(finalFilter)
            .sort(Sorts.ascending("sortOrder", "code"))
            .limit(query.limit.coerceIn(1, 200))
            .into(mutableListOf())
            .map { it.toCatalogCategory() }
    }
}

private class MongoPlatformCatalogFamilyRepository(database: MongoDatabase) : PlatformCatalogFamilyRepository {
    private val collection: MongoCollection<Document> = database.getCollection(MongoCollectionNames.PLATFORM_CATALOG_FAMILIES)
    ...
    override fun findByGlobalFamilyId(globalFamilyId: String): PlatformCatalogFamily? =
        collection.find(eq("globalFamilyId", globalFamilyId.trim().lowercase())).firstOrNull()?.toPlatformCatalogFamily()
    ...
    override fun search(query: PlatformCatalogFamilySearchQuery): List<PlatformCatalogFamily> {
        ...
        if (query.statuses.isNotEmpty()) filters += com.mongodb.client.model.Filters.`in`("status", query.statuses.map { it.name })
        query.query?.trim()?.takeIf { it.isNotBlank() }?.let { text ->
            filters += or(regex("canonicalName", text, "i"), regex("normalizedName", text.lowercase(), "i"), regex("globalFamilyId", text.lowercase(), "i"), regex("aliases", text, "i"))
        }
        val finalFilter = if (filters.isEmpty()) Document() else and(filters)
        return collection.find(finalFilter)
            ...
            .limit(query.limit.coerceIn(1, 200))
            .into(mutableListOf())
            .map { it.toPlatformCatalogFamily() }
    }
}

private fun CatalogCategory.toDocument(): Document =
    Document(MongoDocumentFields.ID, id)
        .append("parentId", parentId)
        .append("code", code)
        .append("name", name)
        .append("normalizedName", normalizedName)
        .append("description", description)
        .append("businessTypeTags", businessTypeTags.toList())
        .append("activityTags", activityTags.toList())
...
private fun Document.toCatalogCategory(): CatalogCategory =
    CatalogCategory(
...
private fun PlatformCatalogFamily.toDocument(): Document =
    Document(MongoDocumentFields.ID, id)
        .append("globalFamilyId", globalFamilyId)
...
private fun Document.toPlatformCatalogFamily(): PlatformCatalogFamily =
