...
object MongoCatalogMappers {
    fun templateToDocument(template: PlatformCatalogTemplate): Document =
        Document(MongoDocumentFields.ID, template.id).append("globalCatalogId", template.globalCatalogId)
            .append("canonicalName", template.canonicalName).append("normalizedName", template.normalizedName)
            .append("type", template.type.name).append("status", template.status.name)
            ...
            .append("attributes", Document(template.attributes)).append("media", template.media.map(::mediaToDocument))

    fun templateFromDocument(document: Document): PlatformCatalogTemplate = PlatformCatalogTemplate(
    ...
    fun itemToDocument(item: OrganizationCatalogItem): Document =
        Document(MongoDocumentFields.ID, item.id).append(MongoDocumentFields.ORGANIZATION_ID, item.organizationId)
            .append("branchId", item.branchId).append("activityId", item.activityId)
            .append("templateId", item.templateId).append("globalCatalogId", item.globalCatalogId)
            .append("localName", item.localName).append("searchableText", item.searchableText)
            .append("type", item.type.name).append("status", item.status.name)
            .append("localPrice", moneyToDocument(item.localPrice)).append("taxProfileId", item.taxProfileId)
            ...
            .append("attributes", Document(item.attributes)).append("media", item.media.map(::mediaToDocument))

    fun itemFromDocument(document: Document): OrganizationCatalogItem = OrganizationCatalogItem(
        id = document.requiredString(MongoDocumentFields.ID),
    ...
    fun requestToDocument(request: CatalogItemRequest): Document =
        Document(MongoDocumentFields.ID, request.id).append(MongoDocumentFields.ORGANIZATION_ID, request.organizationId)
            .append("requestedByUserId", request.requestedByUserId).append("requestedName", request.requestedName)
            .append("normalizedRequestedName", request.requestedName.trim().lowercase())
            .append("requestedType", request.requestedType.name).append("description", request.description)
            ...
            .append("status", request.status.name).append("reviewedByUserId", request.reviewedByUserId)
    ...
    fun requestFromDocument(document: Document): CatalogItemRequest = CatalogItemRequest(
        id = document.requiredString(MongoDocumentFields.ID),
        organizationId = document.requiredString(MongoDocumentFields.ORGANIZATION_ID),
    ...
    fun priceHistoryToDocument(history: CatalogPriceHistory): Document =
        Document(MongoDocumentFields.ID, history.id).append(MongoDocumentFields.ORGANIZATION_ID, history.organizationId)
            .append("catalogItemId", history.catalogItemId).append("oldPrice", moneyToDocument(history.oldPrice))
            .append("newPrice", moneyToDocument(history.newPrice)).append("changedByUserId", history.changedByUserId)
            .append("reason", history.reason).append("changedAt", MongoInstantMapper.toDate(history.changedAt))

    private fun identifierToDocument(identifier: CatalogIdentifier): Document =
        Document("type", identifier.type.name).append("value", identifier.value)
            .append("normalizedValue", identifier.normalizedValue).append("scope", identifier.scope.name)
            .append("status", identifier.status.name).append("source", identifier.source.name)
            .append("isPrimary", identifier.isPrimary)

    private fun identifierFromDocument(document: Document): CatalogIdentifier = CatalogIdentifier(
        type = enumValueOf(document.requiredString("type")),
    ...
    private fun mediaToDocument(media: CatalogMediaAsset): Document =
        Document("id", media.id).append("ownerKind", media.ownerKind.name).append("url", media.url)
            .append("mimeType", media.mimeType).append("status", media.status.name).append("isPrimary", media.isPrimary)
            .append("sortOrder", media.sortOrder)

    private fun mediaFromDocument(document: Document): CatalogMediaAsset = CatalogMediaAsset(
    ...
    private fun moneyToDocument(money: Money): Document =
        Document("amount", MongoDecimalMapper.moneyToDecimal128(money.amount)).append("currency", money.currency.value)
