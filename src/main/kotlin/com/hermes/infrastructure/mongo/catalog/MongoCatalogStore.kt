...
private class MongoPlatformCatalogTemplateRepository(database: MongoDatabase) : PlatformCatalogTemplateRepository {
    private val collection: MongoCollection<Document> = database.getCollection(MongoCollectionNames.PLATFORM_CATALOG_TEMPLATES)
    ...
    override fun update(template: PlatformCatalogTemplate) {
        collection.replaceOne(eq("_id", template.id), MongoCatalogMappers.templateToDocument(template), ReplaceOptions().upsert(false))
    }
    ...
    override fun search(query: CatalogTemplateSearchQuery): List<PlatformCatalogTemplate> {
        ...
        query.query?.trim()?.takeIf { it.isNotBlank() }?.let { text ->
            filters += or(regex("canonicalName", text, "i"), regex("normalizedName", text.lowercase(), "i"), regex("globalCatalogId", text.lowercase(), "i"))
        }
        ...
        return collection.find(finalFilter)
            ...
            .into(mutableListOf())
            .map(MongoCatalogMappers::templateFromDocument)
    }
}

private class MongoOrganizationCatalogItemRepository(database: MongoDatabase) : OrganizationCatalogItemRepository {
    private val collection: MongoCollection<Document> = database.getCollection(MongoCollectionNames.ORGANIZATION_CATALOG_ITEMS)
    ...
    override fun update(item: OrganizationCatalogItem) {
        collection.replaceOne(and(eq("_id", item.id), eq("organizationId", item.organizationId)), MongoCatalogMappers.itemToDocument(item), ReplaceOptions().upsert(false))
    }

    override fun findById(organizationId: String, catalogItemId: String): OrganizationCatalogItem? =
        collection.find(and(eq("_id", catalogItemId.trim()), eq("organizationId", organizationId.trim()))).firstOrNull()?.let(MongoCatalogMappers::itemFromDocument)

    override fun existsByTemplateId(organizationId: String, templateId: String): Boolean =
        collection.find(and(eq("organizationId", organizationId.trim()), eq("templateId", templateId.trim()), ne("status", CatalogItemStatus.REMOVED_FROM_ACCOUNT.name))).limit(1).firstOrNull() != null
    ...
    override fun search(query: OrganizationCatalogSearchQuery): List<OrganizationCatalogItem> {
        ...
        return collection.find(and(filters))
            ...
            .limit(query.limit.coerceIn(1, 100))
            .into(mutableListOf())
            .map(MongoCatalogMappers::itemFromDocument)
    }
}
...
private class MongoCatalogItemRequestRepository(database: MongoDatabase) : CatalogItemRequestRepository {
    ...
    override fun update(request: CatalogItemRequest) {
        collection.replaceOne(eq("_id", request.id), MongoCatalogMappers.requestToDocument(request), ReplaceOptions().upsert(false))
    }
...
private class MongoCatalogIdentifierConflictChecker(database: MongoDatabase) : CatalogIdentifierConflictChecker {
    private val collection: MongoCollection<Document> = database.getCollection(MongoCollectionNames.ORGANIZATION_CATALOG_ITEMS)
    override fun existsLocalIdentifier(organizationId: String, normalizedValue: String, excludeCatalogItemId: String?): Boolean {
        val filters = mutableListOf<org.bson.conversions.Bson>(eq("organizationId", organizationId.trim()), eq("identifiers.normalizedValue", normalizedValue.trim()))
...
class MongoOrganizationCatalogTaxProfileRepository(database: MongoDatabase) : OrganizationCatalogTaxProfileRepository {
    ...
    override fun assignTaxProfile(organizationId: String, catalogItemId: String, taxProfileId: String, updatedAt: Instant): CatalogTaxProfileAssignmentRecord {
        val current = collection.find(and(eq("_id", catalogItemId.trim()), eq("organizationId", organizationId.trim()))).firstOrNull()
        ...
        collection.replaceOne(and(eq("_id", catalogItemId.trim()), eq("organizationId", organizationId.trim())), current, ReplaceOptions().upsert(false))
